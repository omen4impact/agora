...
pub async fn run(config_path: impl AsRef<Path>, _foreground: bool) -> Result<(), NodeError> {
    ...
    let identity = load_or_create_identity(&config.identity.key_file, config.identity.name.as_deref())?;
    ...
    let network_handle = tokio::spawn(async move {
        run_network(config, identity, metrics, dashboard_state, shutdown_token_clone).await
    });
...
async fn run_network(
    ...
    let peer_id = identity.peer_id().parse::<PeerId>()
...
pub async fn status(endpoint: String) -> Result<(), NodeError> {
    ...
    if !response.status().is_success() {
        return Err(NodeError::Network(format!("Status check failed: {}", response.status())));
    }
...
pub async fn generate_identity(output: impl AsRef<Path>, name: Option<String>) -> Result<(), NodeError> {
    let mut identity = Identity::generate()
        .map_err(|e| NodeError::Identity(format!("Failed to generate identity: {}", e)))?;
...
pub async fn discover(
    ...
    let response = client
        .get(&url)
        .query(&[("region", region.as_deref()), ("capability", Some(capability.as_str()))])
        .send()
        .await
        .map_err(|e| NodeError::Network(format!("Failed to connect: {}", e)))?;
    
    if !response.status().is_success() {
        return Err(NodeError::Network(format!("Discovery failed: {}", response.status())));
    }
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        println!("      Load: {}/{} ({:.1}%)", node.current_load, node.max_clients, node.load_percentage());
