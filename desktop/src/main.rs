...
use agora_core::{
    ...
    MixerManager, MixerConfig,
    NetworkNode, NetworkEvent, NetworkCommand,
...
async fn create_room(
    ...
    *room_lock = Some(RoomState {
    ...
    if let Some(cmd_tx) = state.network_command.lock().await.as_ref() {
        let _ = cmd_tx.send(NetworkCommand::JoinRoom { room_id: room.id }).await;
    }
...
async fn join_room(
    ...
    *room_lock = Some(RoomState {
    ...
    if let Some(cmd_tx) = state.network_command.lock().await.as_ref() {
        let _ = cmd_tx.send(NetworkCommand::JoinRoom { room_id: room_id.clone() }).await;
    }
        ...
        while let Ok(event) = event_rx.recv().await {
            match event {
                NetworkEvent::PeerConnected { peer_id, addr } => {
                    let _ = app_handle.emit("peer-connected", serde_json::json!({
                        "peer_id": peer_id.to_string(),
                        "addr": addr.to_string()
                    }));
                }
                NetworkEvent::PeerDisconnected { peer_id } => {
                    let _ = app_handle.emit("peer-disconnected", serde_json::json!({
                        "peer_id": peer_id.to_string()
                    }));
                }
                ...
                NetworkEvent::AudioReceived { peer_id, packet } => {
                    let _ = app_handle.emit("audio-received", serde_json::json!({
                        "peer_id": peer_id.to_string(),
                        "sequence": packet.sequence
                    }));
                }
                NetworkEvent::RoomJoined { room_id, peer_id } => {
                    let _ = app_handle.emit("room-joined", serde_json::json!({
                        "room_id": room_id,
                        "peer_id": peer_id.to_string()
                    }));
                }
                NetworkEvent::RoomLeft { room_id, peer_id } => {
                    let _ = app_handle.emit("room-left", serde_json::json!({
                        "room_id": room_id,
                        "peer_id": peer_id.to_string()
                    }));
                }
                NetworkEvent::NatStatusChanged { is_public } => {
                    let _ = app_handle.emit("nat-status", serde_json::json!({
                        "is_public": is_public
                    }));
                }
                NetworkEvent::Listening(addr) => {
                    let _ = app_handle.emit("listening", serde_json::json!({
                        "addr": addr.to_string()
                    }));
                }
                _ => {}
...
async fn start_audio(
    ...
    audio.start().map_err(|e| format!("Failed to start audio: {}", e))?;
...
async fn get_audio_devices() -> Result<AudioDevices, String> {
    ...
    let input = AudioDevice::input_devices()
        .map_err(|e| format!("Failed to get input devices: {}", e))?;
    let output = AudioDevice::output_devices()
        .map_err(|e| format!("Failed to get output devices: {}", e))?;
    
    Ok(AudioDevices {
        input: input.into_iter().map(|d| AudioDeviceInfo {
            name: d.name,
            is_default: d.is_default,
            channels: d.channels,
            sample_rate: d.sample_rate,
        }).collect(),
        output: output.into_iter().map(|d| AudioDeviceInfo {
            name: d.name,
            is_default: d.is_default,
            channels: d.channels,
            sample_rate: d.sample_rate,
...
async fn add_participant(
    ...
    let mut participants = state.participants.lock().await;
    participants.insert(peer_id.clone(), ParticipantInfo {
        peer_id,
        display_name: None,
        is_mixer: false,
        is_muted: false,
...
async fn set_muted(
    ...
    if let Some(cmd_tx) = state.network_command.lock().await.as_ref() {
        ...
        if let Ok(peer_id_parsed) = peer_id.parse() {
            let _ = cmd_tx.send(NetworkCommand::SendControl { 
...
async fn import_identity(
    ...
    let key_bytes_str = data["key_bytes"].as_str()
        .ok_or_else(|| "Missing key_bytes field".to_string())?;
    let key_bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, key_bytes_str)
