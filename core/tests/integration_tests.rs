...
use agora_core::{
    ...
    Room, RoomConfig, MixerManager, MixerConfig, MixerRole, Participant,
    AudioPipeline, AudioConfig,
    protocol::{AudioPacket, ControlMessage, ControlMessageType, JitterBuffer},
    storage::IdentityStorage,
    mixer::{TopologyMode, ScoreWeights, ParticipantStats},
};
...
async fn test_identity_generation_and_persistence() {
    ...
    let storage = IdentityStorage::with_path(dir.path().to_path_buf()).expect("Failed to create storage");
...
async fn test_mixer_score_calculation() {
    ...
    assert!(score1 > score2, "Higher bandwidth and lower CPU should yield higher score");
...
async fn test_mixer_selection_with_tie() {
    ...
    assert_eq!(mixer1, mixer2, "Same scores should produce same mixer selection");
}
...
async fn test_control_message_serialization() {
    ...
    let encoded_list = participant_list_msg.encode().expect("Failed to encode list");
...
async fn test_network_node_creation() {
    let node = NetworkNode::new(None).await.expect("Failed to create network node");
...
async fn test_multiple_room_participants() {
    ...
    for identity in identities.iter().skip(1) {
        ...
        for other in identities.iter().filter(|i| i.peer_id() != identity.peer_id()) {
...
async fn test_connection_targets() {
    ...
    assert!(sfu_targets.len() <= 1, "In SFU mode, should only connect to mixer");
}
...
mod stress_tests {
    ...
    async fn test_stress_packet_encoding_decoding() {
        ...
        for i in 0..total {
            let size = (i % 960) + 1;
            let frame: Vec<f32> = (0..size).map(|j| (j as f32 / size as f32) * 2.0 - 1.0).collect();
        ...
        assert_eq!(success_count, total, "All packets should encode/decode successfully");
    }
    
    #[tokio::test]
    async fn test_stress_control_messages() {
        let message_types = vec![
            ControlMessageType::JoinRoom { room_id: "room1".to_string() },
            ControlMessageType::LeaveRoom { room_id: "room1".to_string() },
            ...
            ControlMessageType::UpdateInfo { display_name: "Test".to_string() },
        ];
    ...
    async fn test_room_creation_stress() {
        ...
        for i in 0..100 {
            let config = RoomConfig {
                ...
                password: if i % 2 == 0 { Some("password".to_string()) } else { None },
                max_participants: Some((i % 20) + 5),
            };
            
            let room = Room::new(format!("creator_{}", i), config);
            
            assert!(room_ids.insert(room.id.clone()), "Generated duplicate room ID");
            assert_eq!(room.id.len(), 16);
        }
