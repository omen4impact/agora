...
use agora_core::{
    Identity, 
    crypto::{EncryptedChannel, SessionKey, KeyExchange, derive_session_key_from_shared_secret},
    audio_processor::{AudioProcessor, AudioProcessorConfig},
    protocol::{AudioPacket, JitterBuffer},
    codec::{OpusEncoder, OpusDecoder, OpusConfig},
...
async fn test_e2e_key_exchange() {
    ...
    let alice_shared = alice.compute_shared_secret(&bob_public).expect("Alice DH failed");
    let bob_shared = bob.compute_shared_secret(&alice_public).expect("Bob DH failed");
...
async fn test_e2e_encrypted_audio_packet() {
    ...
    let encoded_audio = audio_packet.encode().expect("Failed to encode audio packet");
...
async fn test_e2e_audio_pipeline_encode_decode() {
    ...
    let encoded = processor.process_and_encode(&mut original_frame).expect("Failed to process");
    
    assert!(!encoded.data.is_empty());
    assert!(encoded.sequence > 0);
    
    let decoded = processor.decode_and_process(&encoded.data).expect("Failed to decode");
...
async fn test_e2e_audio_with_echo_cancellation() {
    let mut processor = AudioProcessor::new(AudioProcessorConfig::default()).expect("Failed to create processor");
    
    let far_frame: Vec<f32> = (0..FRAME_SIZE)
        .map(|i| (i as f32 * 0.01).sin() * 0.5)
        .collect();
    
    let mut near_frame: Vec<f32> = far_frame.iter()
        .enumerate()
    ...
    let encoded = processor.process_with_aec(&mut near_frame).expect("Failed to process with AEC");
...
async fn test_e2e_full_voice_chain() {
    ...
    let encoded = processor.process_and_encode(&mut mic_frame).expect("Failed to encode");
...
async fn test_e2e_latency_under_10ms() {
    ...
    for _ in 0..iterations {
        let mut frame: Vec<f32> = (0..FRAME_SIZE).map(|i| (i as f32 * 0.01).sin() * 0.5).collect();
        
        let start = std::time::Instant::now();
        let encoded = processor.process_and_encode(&mut frame).expect("Failed to encode");
        let _decoded = processor.decode_and_process(&encoded.data).expect("Failed to decode");
        total_time += start.elapsed();
    }
    
    let avg_time_us = total_time.as_micros() as f64 / iterations as f64;
    
    assert!(avg_time_us < 10000.0, "Average processing time should be < 10ms, got {}µs", avg_time_us);
}
...
async fn test_e2e_replay_attack_protection() {
    ...
    let _first = channel.decrypt(&encrypted).expect("First decrypt should work");
...
async fn test_e2e_key_rotation() {
    ...
    let encrypted2 = channel.encrypt(message2).expect("Failed to encrypt after rotation");
    
    assert_ne!(encrypted1.ciphertext, encrypted2.ciphertext, "Ciphertexts should differ with different keys");
    
    let decrypted = channel.decrypt(&encrypted2).expect("Failed to decrypt after rotation");
