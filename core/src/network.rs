...
use libp2p::{
    ...
    autonat,
    Transport,
    request_response::{self, Behaviour as RequestResponse, ProtocolSupport, Codec},
};
...
use crate::protocol::{AudioPacket, ControlMessage, ControlMessageType, PROTOCOL_NAME, PROTOCOL_CONTROL, MAX_FRAME_SIZE};
use crate::ice::{IceAgent, IceConfig, Candidate, ConnectionState as IceConnectionState};
...
impl Codec for AudioCodec {
    ...
    async fn read_response<T>(&mut self, proto: &Self::Protocol, io: &mut T) -> io::Result<Self::Response>
    where
        T: AsyncRead + Unpin + Send,
    {
...
pub enum NetworkCommand {
    SendAudio { peer_id: PeerId, packet: AudioPacket },
    BroadcastAudio { room_id: String, packet: AudioPacket },
    SendControl { peer_id: PeerId, message: ControlMessage },
    JoinRoom { room_id: String },
    LeaveRoom { room_id: String },
    ConnectToPeer { addr: Multiaddr },
    Stop,
}
...
impl NetworkNode {
    ...
    pub async fn gather_ice_candidates(&mut self) -> AgoraResult<Vec<String>> {
        ...
        let candidates: Vec<String> = agent.local_candidates()
            .iter()
            .map(|c| c.to_sdp())
            .collect();
    ...
    pub fn ice_candidates(&self) -> Option<Vec<&Candidate>> {
        self.ice_agent.as_ref().map(|a| a.local_candidates().iter().collect())
    }
    
    pub fn ice_connection_state(&self) -> Option<String> {
        self.ice_agent.as_ref().map(|a| {
            match a.state() {
    ...
    pub fn get_selected_ice_connection(&self) -> Option<(std::net::SocketAddr, std::net::SocketAddr)> {
    ...
    pub async fn run(&mut self) {
        let mut command_rx = self.command_rx.take().expect("Command receiver already taken");
    ...
    async fn handle_behaviour_event(&mut self, event: AgoraBehaviourEvent) {
        match event {
            ...
            AgoraBehaviourEvent::Autonat(autonat::Event::StatusChanged { new, .. }) => {
                ...
                tracing::info!("NAT status: {}", if is_public { "Public" } else { "Private" });
                let _ = self.event_tx.send(NetworkEvent::NatStatusChanged { is_public });
            }
            
            AgoraBehaviourEvent::Ping(ping::Event { peer, result, .. }) => {
                match result {
                    Ok(duration) => {
    ...
    async fn send_audio_packet(&mut self, peer_id: PeerId, packet: AudioPacket) {
        let request_id = self.swarm.behaviour_mut().audio_stream.send_request(
            &peer_id,
            packet,
        );
        tracing::trace!("Sent audio packet to {}, request_id: {:?}", peer_id, request_id);
    }
    
    async fn broadcast_audio(&mut self, room_id: &str, packet: AudioPacket) {
        let peers: Vec<PeerId> = self.room_peers
            .get(room_id)
    ...
    async fn send_control_message(&mut self, peer_id: PeerId, message: ControlMessage) {
        let request_id = self.swarm.behaviour_mut().control.send_request(
            &peer_id,
            message,
        );
        tracing::debug!("Sent control message to {}, request_id: {:?}", peer_id, request_id);
    }
