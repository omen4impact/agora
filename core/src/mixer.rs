...
use crate::audio::{AudioFrame, mix_audio};
...
impl MixerManager {
    ...
    pub fn select_mixer(&mut self) -> Option<String> {
        ...
        let mut all_scores: Vec<(String, f64)> = self.participants
            .iter_mut()
            .map(|(id, p)| {
    ...
    pub fn update_local_stats(&mut self, bandwidth_bps: u64, cpu_usage: f32, memory_usage: f32) {
        let local_participant = Participant {
            ...
            stats: ParticipantStats {
                ...
                session_duration: self.mixer_start_time
                    .map(|t| t.elapsed())
...
mod tests {
    ...
    fn test_stability_score() {
        ...
        assert!(score > 0.5, "Low variance should give high stability score, got {}", score);
        ...
        assert!(score_high_variance < score, "High variance should give lower stability score");
    }
