...
use crate::stun::{StunClient, StunBinding};
...
impl Candidate {
    ...
    fn calculate_priority(candidate_type: CandidateType, local_pref: u16, component_id: u16) -> u32 {
        ...
        2u32.pow(24) * type_pref as u32
            + 2u32.pow(8) * local_pref as u32
...
impl Default for IceConfig {
    fn default() -> Self {
        Self {
            stun_servers: ICE_DEFAULT_STUN_SERVERS.iter().map(|s| s.to_string()).collect(),
...
impl IceAgent {
    ...
    pub fn with_turn_server(mut self, url: String, username: Option<String>, password: Option<String>) -> Self {
    ...
    async fn gather_relayed_candidates(&mut self) -> AgoraResult<()> {
        for turn_server in &self.config.turn_servers {
            if turn_server.username.is_none() || turn_server.password.is_none() {
                tracing::warn!("TURN server {} has no credentials, skipping", turn_server.url);
                continue;
            }
            
            tracing::info!("Attempting to create TURN allocation on {}", turn_server.url);
            ...
            let server = if let Ok(addr) = turn_server.url.parse::<SocketAddr>() {
                ...
                let port = turn_server.url.split(':').nth(1).and_then(|p| p.parse().ok());
                crate::turn::TurnServer::from_host(
                    turn_server.url.split(':').next().unwrap_or(&turn_server.url),
                    port,
    ...
    pub async fn perform_connectivity_checks(&mut self) -> AgoraResult<()> {
        ...
        while start_time.elapsed() < timeout {
            let pending_pairs: Vec<(usize, SocketAddr, SocketAddr)> = self.checklist
                .iter()
                .enumerate()
                .filter(|(_, pair)| {
                    matches!(pair.state, CandidatePairState::Waiting | CandidatePairState::InProgress)
                })
    ...
    async fn check_connectivity_impl(&self, local_addr: SocketAddr, remote_addr: SocketAddr) -> AgoraResult<Duration> {
    ...
    fn finalize_selection(&mut self) -> AgoraResult<()> {
        let succeeded_pairs: Vec<&CandidatePair> = self.checklist
            .iter()
    ...
    pub fn get_selected_connection(&self) -> Option<(SocketAddr, SocketAddr)> {
        self.selected_pair.as_ref().map(|p| {
            (p.local.base_addr, p.remote.connection_addr)
        })
    }
}
...
mod tests {
    ...
    fn test_candidate_priority() {
        let host = Candidate::new_host(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1)), 1234), 1);
